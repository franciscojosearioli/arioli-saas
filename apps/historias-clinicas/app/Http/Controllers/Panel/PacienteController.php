...
class PacienteController extends Controller
{
    public function index()
    {
        ...
        return view('panel.pacientes.index', compact('Pacientes'));
    }
    ...
    if ($request->hubo_reingreso == '1' && $request->has('fecha_reingreso') && is_array($request->fecha_reingreso)) {
        ...
        notify()->success('Se ha cargado correctamente el paciente.', 'Nuevo dato cargado');
        // Redirigir a la vista index de pacientes después de guardar
        return redirect()->route('panel.paciente.index');
    }
    ...
    public function edit(Paciente $Paciente)
    {
        ...
        return view('panel.pacientes.edit', compact('Paciente'));
    }
    ...
    if ($request->hubo_reingreso == '1' && $request->has('fecha_reingreso') && is_array($request->fecha_reingreso)) {
        ...
        $paciente->actualizarStatus();
        notify()->success('Se ha actualizado correctamente el paciente.', 'Dato actualizado');
    ...
    public function show(Paciente $Paciente)
    {
        ...
        $Informes     = $Paciente->informes()->with(['tipo', 'profesional', 'recetas'])->orderByDesc('fecha')->get();
        $Medicaciones = $Paciente->medicacion()->orderByDesc('fecha')->get();
        $Agendas      = Agenda::where('paciente_id', $Paciente->id)
                         ->with(['profesional', 'especialidad'])
                         ->orderByDesc('fecha_hora_inicio')
                         ->get();
        $Recetas      = $Informes->flatMap(fn($i) => $i->recetas)->sortByDesc('created_at');
        $Consentimientos = Consentimiento::with(['tipo', 'firmadoPor'])
                            ->where('paciente_id', $Paciente->id)
                            ->orderByDesc('created_at')
                            ->get();

        return view('panel.pacientes.show', compact(
            'Paciente', 'Informes', 'Medicaciones', 'Agendas', 'Recetas', 'Consentimientos'
        ));
    }
    ...
    public function firmarConsentimiento($id)
    {
        abort_if(Gate::denies('paciente_show'), Response::HTTP_FORBIDDEN, '403 Forbidden');

        $paciente = Paciente::findOrFail($id);

        return view('panel.pacientes.consentimiento-firma', compact('paciente'));
    }

    public function guardarConsentimiento(Request $request, $id)
    {
        abort_if(Gate::denies('paciente_edit'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        ...
        $path    = 'firmas/consentimiento/' . $paciente->id . '/firma.png';
        Storage::disk('public')->put($path, $decoded);

        // Generate signed PDF
        $firmaBase64 = base64_encode($decoded);
        $firmadoAt   = Carbon::now();
        ...
        $pdfPath = 'consentimientos/' . $paciente->id . '/consentimiento_firmado.pdf';
        Storage::disk('public')->put($pdfPath, $pdf->output());

        // Update patient record
        $paciente->update([
            'consentimiento_firmado'        => true,
            'consentimiento_firmado_at'     => $firmadoAt,
            'consentimiento_firma_imagen'   => $path,
        ]);

        return redirect()->route('panel.paciente.show', $paciente->id)
            ->with('message', 'Consentimiento firmado y guardado correctamente.');
    }
