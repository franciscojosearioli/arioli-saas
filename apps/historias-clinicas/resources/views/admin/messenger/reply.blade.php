...
<style>
    ...
    .msg-btn-cancel {
        ...
        font-size: 13px; font-weight: 500; text-decoration: none; transition: background .12s;
    }
...
<div class="reply-context">
    ...
    <span class="reply-context-text">
        Respondiendo a <strong>{{ optional($topic->receiverOrCreator())->email ?? '(usuario)' }}</strong>
        &mdash; <em>{{ $topic->subject }}</em>
    </span>
</div>

<form action="{{ route('admin.messenger.reply', $topic->id) }}" method="POST" class="msg-form">
    @csrf
    ...
    <div class="msg-form-actions">
        <a href="{{ route('admin.messenger.showMessages', $topic->id) }}" class="msg-btn-cancel">
            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
