...
@section('title', trans('global.edit') . ' ' . trans('cruds.permission.title_singular'))
...
<div style="max-width:560px; display:flex; flex-direction:column; gap:20px;">
    ...
    <div style="display:flex; align-items:center; gap:16px;">
        ...
        <div>
            ...
            <p style="font-size:13px; color:var(--t2); margin:3px 0 0;">{{ $permission->title }}</p>
        </div>
    </div>
            ...
            <div style="display:flex; flex-direction:column; gap:18px;">
                ...
                <div>
                    <label style="display:block; font-size:13px; font-weight:600; color:var(--t1); margin-bottom:6px;">
                        {{ trans('cruds.permission.fields.title') }} <span style="color:var(--err);">*</span>
                    </label>
                    <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}"
                           type="text" name="title" id="title" value="{{ old('title', $permission->title) }}" required
