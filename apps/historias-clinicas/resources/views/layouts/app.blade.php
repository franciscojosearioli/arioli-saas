...
<div class="layout">
    ...
    <aside id="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            {{-- Navegación aportada por Componentes instalados (ver docs/ARQUITECTURA_MODULAR.md Etapa 4.1) --}}
            @php $navComponentes = app(\App\Platform\Contracts\Services\NavigationInstallerContract::class)->resolverPara(auth()->user()); @endphp
            @if(count($navComponentes))
            <div class="nav-section">Componentes</div>
            @foreach ($navComponentes as $item)
            <a href="{{ route($item->route) }}" data-title="{{ $item->label }}"
