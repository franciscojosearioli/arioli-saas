<div style="display:flex; align-items:center; justify-content:space-between; padding:14px 20px; border-top:1px solid var(--card-border);">
    <span style="font-size:12.5px; color:var(--text-muted);">
        @if($licenses->total() > 0)
            Mostrando {{ $licenses->firstItem() }} a {{ $licenses->lastItem() }} de {{ $licenses->total() }} licencia{{ $licenses->total() === 1 ? '' : 's' }}{{ !empty($search) ? ' para "' . e($search) . '"' : '' }}
        @else
            Sin resultados
        @endif
    </span>
        ...
        <div style="display:flex; gap:4px;">
            ...
            @else
                <a href="javascript:void(0)"
                   onclick="fetchLicenses({{ $licenses->currentPage() - 1 }})"
