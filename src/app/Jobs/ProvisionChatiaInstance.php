...
class ProvisionChatiaInstance implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            if ($exitCode === 0) {
                Log::info("{$this->slug} instance provisioned successfully", [
                    'tenant_id' => $this->tenantId,
                    'database'  => "chatia_{$this->tenantId}",
                    'output'    => $output,
                ]);
            } else {
        ...
        try {
            Mail::to($this->adminEmail)
                ->send(new \App\Mail\ChatiaWelcomeMail(
                    $this->adminName,
                    $this->adminEmail,
                    $this->tenantId,
                    $this->adminPassword,
            ...
            Log::error('ChatiaWelcomeMail failed', [
                'tenant_id' => $this->tenantId,
                'error'     => $e->getMessage(),
            ]);
        }
