...
class ProvisionTurnosInstance implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            if ($exitCode === 0) {
                Log::info("{$this->slug} instance provisioned successfully", [
                    'tenant_id' => $this->tenantId,
                    'database'  => "turnos_{$this->tenantId}",
                    'output'    => $output,
                ]);
            } else {
        ...
        try {
            Mail::to($this->adminEmail)
                ->send(new \App\Mail\TurnosWelcomeMail(
                    $this->adminName,
                    $this->adminEmail,
                    $this->tenantId,
                    $this->adminPassword,
            ...
            Log::error('TurnosWelcomeMail failed', [
                'tenant_id' => $this->tenantId,
                'error'     => $e->getMessage(),
            ]);
        }
