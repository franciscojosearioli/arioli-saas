...
class ProvisionTallerProInstance implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            if ($exitCode === 0) {
                Log::info("{$this->slug} instance provisioned successfully", [
                    'tenant_id' => $this->tenantId,
                    'database'  => "{$this->slug}_{$this->tenantId}",
                    'output'    => $output,
                ]);
            } else {
        ...
        try {
            Mail::to($this->adminEmail)
                ->send(new \App\Mail\TallerProWelcomeMail(
                    $this->adminName,
                    $this->adminEmail,
                    $this->tenantId,
                    $this->adminPassword,
            ...
            Log::error('TallerProWelcomeMail failed', [
                'tenant_id' => $this->tenantId,
                'error'     => $e->getMessage(),
            ]);
        }
