...
class ProvisionChatiaDirectly extends Command
{
    protected $signature = 'provision:chatia-direct {tenant_id} {admin_name} {admin_email} {admin_password}';
    protected $description = 'Provision Chatia tenant — clones schema from chatia_default, creates admin user';

    // Base instance con el schema completo de F0 (ver docs/chatia-architecture.md
    // §5.2) — mismo patrón que ProvisionTurnosDirectly/ProvisionSubastasDirectly:
    // clonar de una base viva en vez de mantener SQL hardcodeado en paralelo
    // (el patrón de ProvisionTallerProDirectly, que causó drift real entre el
    // SQL hardcodeado y las migraciones reales — ver memoria del proyecto).
    private const SOURCE_DB = 'chatia_default';
    ...
    public function handle(): int
    {
        ...
        $dbName        = 'chatia_' . $tenantId;
        $sourceDb      = self::SOURCE_DB;

        $this->info("Provisionando Chatia tenant: {$tenantId}");

        $latestVersion = AppVersion::latestFor('chatia')?->version ?? '1.0.0';
        ...
        try {
            ...
            if (!$sourceExists) {
                $this->error("La base de datos de referencia '{$sourceDb}' no existe. Verificá que el contenedor saas_chatia esté corriendo y migrado.");
                return 1;
            }
            ...
            // provisionado por compra (ver apps/chatia SetupWizardController).
            ...
            $plan = \App\Models\Plan::sellable()
                ->whereHas('product', fn($q) => $q->where('slug', 'chatia'))
                ->orderBy('price')->first();

            if ($plan) {
                $existing = License::where('tenant_id', $tenantId)
                    ->whereHas('plan.product', fn($q) => $q->where('slug', 'chatia'))
                    ->first();
            ...
            $this->info("✅ Chatia tenant '{$tenantId}' provisionado exitosamente!");
